...
impl<'a> IndexSync<'a> {
    pub fn from_schema(partition: &'a TxPartition, txid: u64, schema: &'a Schema) -> Self {
        let Schema {
            table_name,
            column_defs,
            indexes,
    ...
    pub fn new(
        ...
        let Schema {
            ...
            indexes,
            ..
        } = fetch_schema(schema_partition, table_name)?
        ...
        Ok(Self {
            data_partition: index_partition,
            txid,
            table_name,
            columns,
    ...
    pub async fn insert(&self, data_key: &[u8], row: &DataRow) -> Result<(), Error> {
    ...
    pub async fn delete(&self, data_key: &[u8], row: &DataRow) -> Result<(), Error> {
        for index in self.indexes.iter() {
    ...
    fn insert_index_data(&self, index_key: &[u8], data_key: &[u8]) -> Result<(), Error> {
